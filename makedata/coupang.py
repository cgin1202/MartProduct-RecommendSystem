...
def normalization(dicts):
    ...
    for key, value in dicts.items():
        result[key] = (value-min_val)/bottom
    return result

def flattening(dicts, m, k):
    result = {}
    for key, value in dicts.items():
        result[key] = (value+k)/2
    return result

def crawling_product(driver, product):
    driver.get('https://www.coupang.com/np/search?component=&q='+product+'&channel=user')
    html = driver.page_source
    soup = BeautifulSoup(html, 'html.parser')
    notices = soup.select('span.rating-total-count')
    num = 0
    for n in notices:
        num += int(n.text[1:-1])
    return product, num

def crawling_coupang(products):
    driver = webdriver.Chrome('/Users/paki1019/Documents/westdoor456/westdoor456/makedata/chromedriver')
    result = {}
    for product in products:
        key, value = crawling_product(driver, product)
        result[key] = value
...
mydb = dbconnection()
mycol = mydb.dashboard_product
results = mycol.find()
products = []
for result in results:
    products.append(result['product_name'])

#product_stars = (crawling_coupang(products))
#print(product_stars)
product_stars = {'라면': 272091, '사과': 36592, '피자': 2909, '버거': 8746, '세제': 237336, '감자': 14108, '만두': 76387, '삼겹살': 10941, '옥수수': 4987, '김': 144214}
posibillity = flattening(normalization(product_stars), 1.5, 0.4)
#makeTestCustomer(mydb, 100)
setTestData(mydb, posibillity)
